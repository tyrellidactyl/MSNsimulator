%% Case 1: Constant Source strength
%-------------------------------------------------------------------------%
% A simple Forward in Time and Central in Space algorithm for solving the
% Gaussian Concentration 

%save: s will save at each ts (3d ind .mat files)
%-------------------------------------------------------------------------%
...
dx=0.25; dy=0.25; dz=0.25; dt = 0.005;
...
string1             = 'Data Output';
choice              = questdlg(qstring,string1,'Yes','No',options);
...
%     title(ch_t,'fontweight','bold','fontsize',sz);
    ...
    if c_sv==1
        %   Save data files at time steps
        fl_C  = strcat('Conc_Case1_TimeStep_',num2str(t-1),'.mat');
        save(fl_C,'C_r')
        
%         %   Save Velocity fields, uncomment these if varying velocities
%         %   are used
%         fl_u  = strcat('U_Case1_TimeStep_',num2str(t-1),'.mat');
%         save(fl_u,'U_r')
%         
%         fl_v  = strcat('V_Case1_TimeStep_',num2str(t-1),'.mat');
%         save(fl_v,'V_r')
%         
%         fl_w  = strcat('W_Case1_TimeStep_',num2str(t-1),'.mat');
%         save(fl_w,'W_r')
    else
        % Do nothing
    end
...
% Since constant velocities are used, files are saved at one time only
