...
leader = pioneerwaypointclass2(); % Instantiate the object
...
% declare starting angle and final angle for polar function (degrees / 10)
angle_1 = 0;
angle_2 = 36;
...
while (k == 1)
    ...
    if (c_v == 0) % UGV must be resting at the origin before following the set path
        ...
        n = size(leader.waypoints,1); % Number of waypoints
        
        tl = 1;

        while (leader.wpt_index ~= n)

            leader.display_status(); % Display odometry data in the command window

            % Collect current odometry status
            c_x = leader.x_odom;
            c_y = leader.y_odom;
            c_v = leader.msg_twist.Linear.X;
    ...
    % Display odometry data in command window while the UGV goes to the origin
